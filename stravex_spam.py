...
def friend_request_packet(K, V, uid):
    fields = {
        1: 10,
        2: {
            1: int(uid),
...
def _profile_like_packet(uid, action):
    uid_bytes = EnC_Uid(int(uid), Tp='Uid')
    uid_len = len(uid_bytes) // 2
    inner = f"0a{uid_len:02x}{uid_bytes}10{action:02x}"
    inner_len = len(inner) // 2
    outer = f"0801{inner_len:02x}{inner}"
    return outer

def like_player_profile(uid, K, V):
    Pk = _profile_like_packet(uid, 6)
    return GeneRaTePk(Pk, '0f15', K, V)

def like_player_profile_v2(uid, K, V):
    Pk = _profile_like_packet(uid, 5)
    return GeneRaTePk(Pk, '0f15', K, V)

def like_player_profile_v3(uid, K, V):
    uid_bytes = EnC_Uid(int(uid), Tp='Uid')
    uid_len = len(uid_bytes) // 2
    inner = f"0a{uid_len:02x}{uid_bytes}1005"
    inner_len = len(inner) // 2
    outer = f"0802{inner_len:02x}{inner}"
    return GeneRaTePk(outer, '0f15', K, V)
